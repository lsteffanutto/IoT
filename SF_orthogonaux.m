...
NbPaquet=10^1;            % Nombre de paquet transmis (utiliser pour tracer des courbes de performances BER par exemple)
...
for PR=1:lPR1
    for numSNR = 1 : length(SNRdB)
        ...
        for numPaquet = 1:NbPaquet
            ...
            Psf2 = mean(abs(Ssf2).^2);
            Psf1 = Psf2.*(10^-(PR1(PR)/10));
            sigRx = [sqrt(Psf1).*Ssf1 + sqrt(Psf2).*Ssf2(1:length(Ssf1)) sqrt(Psf2).*Ssf2(length(Ssf1)+1:end)];
